...
# 0 = Secure Boot on, 1 = off, 2 = mokutil couldn't tell. An unsigned module
# under Secure Boot leaves nouveau blacklisted AND nvidia not loaded, which is
# no KMS driver at all rather than a slow one.
...
# Separate file, not a rewrite of sources.list, and no non-free-firmware or
# apt warns about a component configured twice. debian mode only.
...
# local.conf is untracked: GBM_BACKEND=nvidia-drm on a card-less machine breaks
# rendering outright. Rewritten between markers so re-runs do not stack.
...
step_nvidia() {
    ...
    # linux-headers-amd64, not $(uname -r): tracks kernel upgrades, so dkms still
    # has headers after the next one. nvidia-open is DKMS-built too.
    ...
    local unit
    # Shipped but not enabled; NVreg_PreserveVideoMemoryAllocations has nothing
    # to hook into without them.
