...
# Run INSIDE the test VM, once, to make it debuggable from the host. None of
# this belongs on a real machine, which is why it is not a bootstrap step.
#
...
# Hyprland logs to $XDG_RUNTIME_DIR, wiped when the last session ends — exactly
# when a crash is worth reading. Lingering keeps it.
